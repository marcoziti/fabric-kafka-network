...
function generateCerts (){
	...
	echo "##########################################################"
	target=${FABRIC_CFG_PATH}/crypto-config
	if find "$target" -mindepth 1 -print -quit 2>/dev/null | grep -q .; then
		echo "# ${FABRIC_CFG_PATH}/crypto-config) not empty, please check if certs are already generated."
		echo "# Will exit."
		exit 1
	fi
	...
	CRYPTOGEN_CMD="docker run --rm -v ${FABRIC_ROOT}:/fabric -v ${FABRIC_ROOT}/crypto-config:/fabric/config/crypto-config ${FABRIC_DOCKER_IMAGE_NAME} /usr/local/bin/cryptogen generate --config=/fabric/config/crypto-config.yaml --output=/fabric/crypto-config"
	echo "# ${CRYPTOGEN_CMD}"
	eval "${CRYPTOGEN_CMD}"
}
...
function generateChannelArtifacts() {
	...
	CONFIGTXGEN_CMD="docker run --rm --env FABRIC_CFG_PATH=/fabric/config  -v ${FABRIC_ROOT}:/fabric -v ${FABRIC_ROOT}/crypto-config:/fabric/config/crypto-config $FABRIC_DOCKER_IMAGE_NAME /usr/local/bin/configtxgen -profile TwoOrgsChannel -outputCreateChannelTx /fabric/channel-artifacts/channel.tx -channelID $CHANNEL_NAME"
	...
	CONFIGTXGEN_CMD="docker run --rm --env FABRIC_CFG_PATH=/fabric/config  -v ${FABRIC_ROOT}:/fabric -v ${FABRIC_ROOT}/crypto-config:/fabric/config/crypto-config $FABRIC_DOCKER_IMAGE_NAME /usr/local/bin/configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate /fabric/channel-artifacts/Org1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org1MSP"
	echo "# ${CONFIGTXGEN_CMD}"
	eval "${CONFIGTXGEN_CMD}"
	...
	CONFIGTXGEN_CMD="docker run --rm --env FABRIC_CFG_PATH=/fabric/config -v ${FABRIC_ROOT}:/fabric -v ${FABRIC_ROOT}/crypto-config:/fabric/config/crypto-config $FABRIC_DOCKER_IMAGE_NAME /usr/local/bin/configtxgen -profile TwoOrgsChannel -outputAnchorPeersUpdate /fabric/channel-artifacts/Org2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg Org2MSP"
	echo "# ${CONFIGTXGEN_CMD}"
	eval "${CONFIGTXGEN_CMD}"
	...
	CONFIGTXGEN_CMD="docker run --rm --env FABRIC_CFG_PATH=/fabric/config -v ${FABRIC_ROOT}:/fabric -v ${FABRIC_ROOT}/crypto-config:/fabric/config/crypto-config $FABRIC_DOCKER_IMAGE_NAME /usr/local/bin/configtxgen -profile TwoOrgsOrdererGenesis -outputBlock=/fabric/channel-artifacts/genesis.block"
	echo "# ${CONFIGTXGEN_CMD}"
	eval "${CONFIGTXGEN_CMD}"
}
...
function launchDockerComposeNetwork(){
	...
	for c in $(docker ps -aq); do
		DOCKER_LOG_CMD="docker container logs ${c} --tail 5"
		echo "# ${DOCKER_LOG_CMD}"
		eval "${DOCKER_LOG_CMD}"
	done
}
